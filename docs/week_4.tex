...
\item Note that `\emph{IL}' as well as `\emph{both}' lead to \alert{\emph{incomplete phase extraction}} (\textbf{formalise this concept!}) and hence somewhat meaningless results 
...
\begin{itemize}
\item Let $\mathsf{M}_j$ be the mismatch between $\ket{j} \ket{\Psi(j)}$ and $\hat{Q}_\Psi \ket{j}\ket{0}$
\item Define \alert{$\mu \equiv \text{Mean}(\mathsf{M}_j)$} and \alert{$\sigma \equiv \text{STDEV}(\mathsf{M}_j)$}
\end{itemize}
\item SAM tends to low $\mu$ but high $\sigma$, corresponding to the encoding working well for most input states and quite badly for some outliers
\item DEFINE A NEW METRIC TRYING TO QUANTIFY INCOMPLETE PHASE EXTRACTION! NORM OF VECTOR? CHI SQUARED ERROR?
