...
\item To reduce the parameter space, the \alert{three-parameter operator} 
\begin{equation}
...
\begin{equation}
\mathsf{M}= 1 - \sqrt{|\braket{\psi_\text{target}| \psi_\text{out}}|}
\end{equation}
\item There are two ways to train the QCNN on input data:\footnote{Once can also train the QCNN to produce a target distribution independent of the input register, which is equivalent to constructing $\hat{U}_A$}
\begin{enumerate}
