...
\begin{equation}
\mathsf{WIM}(x,y) =  \left\vert 1 - \sum_n w_n x_n y_n \right \vert, 
\end{equation}
where the weights $w_n \in \mathbb{R}_{+}$ may be recomputed between epochs

\end{itemize}
IDEA: recompute $w_n$ based on mismatch?? -> let $w_n$ be (normalised) mismatch of $n$th input state? ; OR maybe base weighting on size of "plateau" ? (need to normalise $w x$!) ; or maybe on gradient ??
\end{frame}
