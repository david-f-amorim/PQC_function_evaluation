...
\centering
\includegraphics[width=\textwidth]{im/phase_loss_comp_linear_m4}
\caption{Comparison of loss functions for $\Psi \sim \sin x$ and $m=4$. Target in black; rounded target dashed in grey.}
\end{figure}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}
\frametitle{Initial Tests}
look into effects of increasing L and or epochs...
...
\begin{column}{0.5\textwidth}
\begin{itemize}
\item right now: using -l MM -TS -r -L 3 -e 600 for different m values ... not learning even for m=3 ...
\item  at least MM quickly stagnates ...
\item take-aways so far:
\begin{enumerate}[(a)]
\item -TS is far superior to individual training 
\item CE worse than MM ; L1 worse than MM but better than CE 
\end{enumerate}
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}  
...
\begin{itemize}
\item more fully explore PQC parameter space ... 
\item look into barren plateau mitigation (layer-by-layer training....) 
\end{itemize}
\end{frame}
